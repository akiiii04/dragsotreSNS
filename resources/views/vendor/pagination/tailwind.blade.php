@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center">
            ...
            <div class="mx-auto">
                <span class="relative z-0 inline-flex shadow-sm rounded-md pl-10">
                    {{-- Previous Page Link --}}
                    @if ($paginator->onFirstPage())
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if(($paginator->currentPage() >=($page-2) && $paginator->currentPage() <=($page+2)) 
                                    || ($paginator->onFirstPage() && $paginator->currentPage() >=($page-4))
                                    || ($paginator->currentPage() == 2 && $paginator->currentPage() >=($page-3))
                                    || ($paginator->currentPage() == $paginator->lastPage() && $paginator->currentPage() <=($page+4))
                                    || ($paginator->currentPage() == ($paginator->lastPage())-1 && $paginator->currentPage() <=($page+3)))
                                    @if ($page == $paginator->currentPage())
                                        <span aria-current="page">
                                            <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white bg-sky-400 border border-sky-400 cursor-default leading-5">{{ $page }}</span>
                                        </span>
                                    @else
